...
static void printbytes(const unsigned char* x, unsigned long long xlen)
{
    ...
    for (i = 0; i < xlen; i++) sprintf(outs + 2 * i, "%02x", x[i]);
    outs[2 * xlen] = 0;
    hal_send_str(outs);
}
...
static void printpoly32(const poly* x)
{
    char outs[4 * N + 1];
    unsigned long long i;
    for (i = 0; i < N; i++) sprintf(outs + 4 * i, "%04x", x->coeffs[i]);
    outs[4 * N] = 0;
    hal_send_str(outs);
}
static void printpoly16(const poly* x)
{
    ...
    char outs[4 * N + 1];
    unsigned long long i;
    for (i = 0; i < 2 * N; i++) sprintf(outs + 2 * i, "%02x", xp[i]);
    outs[4 * N] = 0;
    hal_send_str(outs);
}
...
int test_double_moduli_ntt()
{
    ...
    hal_send_str("poly_double_ntt_precomp c:");
    printpoly32(&c);
    poly_double_ntt(&b);
    hal_send_str("poly_double_ntt b:");
    printpoly32(&b);
